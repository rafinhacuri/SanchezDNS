//nolint:dupl // endpoint distinto do fetch.Reversos, compartilha apenas o boilerplate de validação
package fetch
...
func Orfaos(c *gin.Context) {
	...
	orfaos, err := records.FetchReversosOrfaos(ctx, util.PdnsClient(), zona)
	if err != nil {
		log.Println(err)
