...
const { data, error, refresh } = await useFetch<{ zones: number, records: number, uptime: string, users: number, status: string, totalQueries: number, failedQueries: number, udpQueries: number, tcpQueries: number, serverId: string, startedAt: string }>('/server/api/statistics', { method: 'GET', query: { connection: optionSelected.value } })
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <div>
            ...
            <div class="text-xl font-semibold">
              {{ data.failedQueries.toLocaleString() }}
            </div>
          </div>
          <div>
            <div class="text-sm text-gray-500">
